...
def test_setup_creds_file():
    ...
    copy_creds_file_path = os.path.join(platform_config["aws_directory"], f'{platform_config["creds_file_name"]}.copy')
    ...
    try:
        ...
        if os.path.exists(creds_file_path) and os.path.exists(copy_creds_file_path):
            # copy creds
            with open(copy_creds_file_path, 'rb') as src, open(creds_file_path, 'wb') as dst: dst.write(src.read())
...
def test_setup_config_file():
    ...
    with open(config_file_copy, 'rb') as src, open(config_file_path, 'wb') as dst: dst.write(src.read())
